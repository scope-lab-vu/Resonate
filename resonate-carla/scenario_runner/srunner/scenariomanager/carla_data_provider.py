...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def on_carla_tick():
        ...
        for actor in CarlaDataProvider._actor_velocity_map:
            if actor is not None and actor.is_alive:
                ...
                ego_vel.append(calculate_velocity(actor))

        for key in CarlaDataProvider._actor_velocity_map:
                #CarlaDataProvider._actor_velocity_map[key] = calculate_velocity(key)
                vel.append(CarlaDataProvider._actor_velocity_map[key])
        ...
        for actor in CarlaDataProvider._actor_location_map:
            if actor is not None and actor.is_alive:
                ...
                ego_location.append(actor.get_location())

        for key in CarlaDataProvider._actor_location_map:
                CarlaDataProvider._actor_location_map[key] = key.get_location()
                val = key.get_location()
                #print(CarlaDataProvider._actor_location_map[key])
                #location.append(val)
        #print(location)

        #print(CarlaDataProvider._actor_transform_map)
        for actor in CarlaDataProvider._actor_transform_map:
            if actor is not None and actor.is_alive:
                CarlaDataProvider._actor_transform_map[actor] = actor.get_transform()
                location.append(actor.get_transform())
        #print(location)
                #print(CarlaDataProvider._actor_transform_map[actor])

        #print(ego_location)

        #return vel, location,CarlaDataProvider._actor_velocity_map[actor],CarlaDataProvider._actor_location_map[actor]
    ...
    def get_location(actor):
        ...
        for key in CarlaDataProvider._actor_location_map:
            #data.append(CarlaDataProvider._actor_location_map[key].get_transform().location)
            if key.id == actor.id:
