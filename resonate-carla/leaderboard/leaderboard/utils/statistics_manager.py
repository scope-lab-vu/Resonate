...
class StatisticsManager(object):
    ...
    def compute_route_statistics(self, config, duration_time_system=-1, duration_time_game=-1):
        ...
        print(route_record.meta['route_length'])
            ...
            if node.list_traffic_events:
                ...
                for event in node.list_traffic_events:
                    if event.get_type() == TrafficEventType.COLLISION_STATIC:
                        ...
                        #route_record.infractions['route_completed'].append(event.get_message())
                        score_route = 100.0
                        target_reached = True
        ...
        # update route scores
        #route_record.scores['route_finished'] = score_route
    ...
    def compute_global_statistics(self, total_routes):
        ...
        if self._registry_route_records:
            for route_record in self._registry_route_records:
                ...
                for key in global_record.infractions.keys():
                    #print(route_record.meta['route_length'] / 1000.0)
        ...
        print(round(float(route_length_kms),2))
        ...
        return global_record, round(float(route_length_kms),2)
    ...
    @staticmethod
    def save_global_record(route_record, distance, sensors, endpoint,filename,config):
        data = fetch_dict(endpoint)
        if not data:
            data = create_default_json_msg()
        ...
        collision = 0
        if(stats_dict['infractions']['collisions_vehicle'] > 0):
            collision +=1
        if(stats_dict['infractions']['collisions_pedestrian'] > 0):
            collision+=1
        ...
        myvalues = [{'avg. route score': '{:.3f}'.format(stats_dict['scores']['score_route']),
                    'collision':collision, 'outside_route_lanes':'{:.3f}'.format(stats_dict['infractions']['outside_route_lanes']),
                    ...
                    'collision_with_vehicle':'{:.3f}'.format(stats_dict['infractions']['collisions_vehicle']),
                    'collision_with_pedestrian':'{:.3f}'.format(stats_dict['infractions']['collisions_pedestrian']),
                    'route_deviation':'{:.3f}'.format(stats_dict['infractions']['route_dev']), 'distance_travelled':distance}]
