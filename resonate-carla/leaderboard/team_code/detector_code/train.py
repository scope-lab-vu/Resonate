...
# path to USB
USBPath = "/home/scope/Carla/autopilot_Carla_ad/sample_data/center/"
# list of folders used in training
trainingFolders = ["noon1","noon2","noon3","noon4","noon5","noon6"]
...
Working_folder = 'center-1024'#experiment
...
def load_data():
    ...
    img_train, img_test = np.array(csv_input[0:len(csv_input)-400]), np.array(csv_input[len(csv_input)-400:len(csv_input)])
...
def CreateModels(n_latent=1024, sample_enc=sample_func, beta=1, C=0):
    model = Sequential()
...
#     x = LeakyReLU(0.1)(x)
    ...
    return autoencoder, encoder,decoder, z_log_var, scheduler


#Train function to fit the data to the model
def train(X,autoencoder,call1,scheduler):
    X_train,X_test = X
    ...
    callbacks_list = [EarlyStopping(monitor='val_loss', mode='min', patience=20, verbose=0),scheduler,call1,checkpoint] #callback list
...
if __name__ == '__main__':
    ...
    inp = load_data()
    print("created model")
    autoencoder,encoder,decoder,z_log_var,scheduler = CreateModels()# Running the autoencoder model
    print("training model")
    train(inp,autoencoder,call1,scheduler)#Train the model with the data
    print("testing model")
